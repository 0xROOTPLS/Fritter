...
At runtime, Fritter minimizes the executable footprint of the loader. The loader is partitioned into individually encrypted functions, each with its own PE section, XOR key, and per-function dispatcher. A function is decrypted on call, executed, wiped on return; only one function's bytes are in plaintext at any given moment. There is no vectored-exception handler, no page-fault storm, no per-page master key, and no residual sliding window - the earlier VEH model has been retired in favour of this dispatch model.
...
On Windows, MSVC is the recommended toolchain. It lays each loader function into its own page-aligned PE section, which is what the N>1 per-function dispatch model needs to individually encrypt/decrypt each hot routine. mingw currently emits everything into a single `.text` and therefore runs the N=1 fallback (one entry covering the entire loader with one XOR key) — functionally identical to MSVC output, but only one dispatcher of polymorphism instead of many. If you don't have Visual Studio, build under WSL with `Makefile.linux`; it cross-compiles the Windows loader via mingw-w64 and produces a working artifact.
...
3. **Dispatch shim.** Replaces the earlier VEH sliding-window shim. Flips the loader region RW->RWX once, then hands control to the loader entry, which under N>1 dispatch is routed through per-function thunks. Each thunk calls a per-function dispatcher that decrypts its function on entry, runs it, and wipes on return. Every dispatcher's opcode layout varies per build (see CHANGELOG v1.3 for the axes: register rotation, XOR loop shape, junk-NOP sites, push/pop shuffle, r10/r11 swap, and so on).
...
5. **Cleanup.** Wipes loader pages with a per-build byte pattern, erases the instance, and exits via thread or process termination per `-x`. No VEH handler to remove and no VEH context struct to scrub - the dispatch shim leaves neither behind.

Residual footprint after execution is one small RWX page where the dispatch shim ran (its own return path must execute from that page, so it cannot self-wipe). In thread mode the mapped PE section is intentionally left intact so CRT callbacks have continuations.
