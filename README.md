...
On Windows, MSVC is the recommended toolchain. It lays each loader function into its own page-aligned PE section, which keeps hot loops from straddling a 4 KiB boundary under the VEH sliding window — worth roughly an order of magnitude in time-to-execution versus the mingw-built loader. If you don't have Visual Studio, build under WSL with `Makefile.linux`; it cross-compiles the Windows loader via mingw-w64 and produces a working artifact, just a slower one.
